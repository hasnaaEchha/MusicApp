app.controller('MusicCtrl',
  ["$sce",'$scope','$timeout','listArtist','identity', function ($sce, $scope,$timeout,listArtist,identity) {
    $scope.API = null;
    $scope.active = 0;
      $scope.identity=identity;
      var i=listArtist.getArtistes('hi','music').then(function(result){
      ...
      $scope.search=function(){
          ...
          listArtist.getArtistes($scope.selected,'music').then(function(result){
              ...
              $scope.changeList=$scope.artists[0];
              setTimeout( function()
              {
                  $scope.$apply();
              }, 1000 );
